...
package io.filpool.handler;

import com.alibaba.fastjson.JSON;
import com.auth0.jwt.exceptions.JWTDecodeException;
import io.filpool.framework.common.api.ApiCode;
import io.filpool.framework.common.api.ApiResult;
import io.filpool.framework.common.exception.*;
import io.filpool.framework.core.bean.RequestDetail;
import io.filpool.framework.core.util.RequestDetailThreadLocal;
import io.filpool.system.exception.SysLoginException;
import io.filpool.system.exception.VerificationCodeException;
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(FILPoolException.class)
    @ResponseStatus(value = HttpStatus.OK)
    public ApiResult<String> handleFilPoolExceptionHandler(FILPoolException exception) {
        if (StringUtils.equals(exception.getMsgKey(),"user.account.exits"))
            return ApiResult.fail(ApiCode.ACCOUNT_EXITS.getCode(),messageSource.getMessage(exception.getMsgKey(), null, LocaleContextHolder.getLocale()));
        else if (StringUtils.equals(exception.getMsgKey(),"user.account.not-exits"))
            return ApiResult.fail(ApiCode.ACCOUNT_NOT_EXITS.getCode(),messageSource.getMessage(exception.getMsgKey(), null, LocaleContextHolder.getLocale()));
        else if (StringUtils.equals(exception.getMsgKey(),"user.paypwd.not-set"))
            return ApiResult.fail(ApiCode.ACCOUNT_PAY_PWD_NOT_SET.getCode(),messageSource.getMessage(exception.getMsgKey(), null, LocaleContextHolder.getLocale()));
        else if (StringUtils.equals(exception.getMsgKey(),"google.verify.not-bind"))
            return ApiResult.fail(ApiCode.GOOGLE_VERIFY_NOT_BIND.getCode(),messageSource.getMessage(exception.getMsgKey(), null, LocaleContextHolder.getLocale()));
        else
            return ApiResult.fail(messageSource.getMessage(exception.getMsgKey(), null, LocaleContextHolder.getLocale()));
    }

    @ExceptionHandler(NeedLoginException.class)
    @ResponseStatus(value = HttpStatus.OK)
    public ApiResult<String> handleNeedLoginExceptionHandler(NeedLoginException e){
        //登录异常
        return ApiResult.fail(401,messageSource.getMessage("token.error", null, LocaleContextHolder.getLocale()));
    }

//    @ExceptionHandler(SendRawException.class)
//    @ResponseStatus(value = HttpStatus.OK)
//    public ApiResult<String> handleSendRawExceptionHandler(SendRawException exception) {
//        return ApiResult.fail(exception.getMsg());
//    }
