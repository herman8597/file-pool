...
public class UserServiceImpl extends BaseServiceImpl<UserMapper, User> implements UserService {
    ...
    public Paging<UserVo> getUserPageList(SysUserPageRequest request) throws Exception {
        ...
        if (!ObjectUtils.isEmpty(userPageInfo.getRecords())) {
            for (User record : userPageInfo.getRecords()) {
                ...
                //查询账户算力总额
                BigDecimal totalAmount = powerOrderService.getBaseMapper().sumEffectAmount(record.getId());
    ...
    public void sendRegisterCode(SendRegisterCodeRequest request) throws Exception {
        ...
        if (AccountUtil.checkEmail(request.getAccount())) {
            boolean sendSmsCode = sendEmailSMSUtil.sendEmailMsg(request.getAccount(), smsCode);
            if (!sendSmsCode) {
    ...
    public void sendEmailCode(String email) throws Exception {
        ...
        boolean sendSmsCode = sendEmailSMSUtil.sendEmailMsg(email, smsCode);
        if (!sendSmsCode) {
            throw new FILPoolException("verify.sms.email.sendFailed");
        }
...
//        user.setPayPassword(payEncrypt);
    ...
    public UserInfoVo getUserInfo(Long userId) throws Exception {
        ...
        userInfoVo.setMobile(PhoneUtil.desensitize(one.getMobile()));
        userInfoVo.setEmail(one.getEmail());
...
//        userInfoVo.setLevelId(one.getLevelId());
    ...
    public UserVo queryDescById(Long id) throws Exception {
        ...
        if (ObjectUtils.isNotEmpty(one)){
            ...
            //查询账户算力总额
            BigDecimal totalAmount = powerOrderService.getBaseMapper().sumEffectAmount(id);
    ...
    public Paging<PledgeListRequest> getPledgePageList(SysUserPageRequest sysUserPageRequest){
        ...
        objectPage.setTotal(pledgeList.size());

        return new Paging<>(objectPage);
    }

    @Override
    public Paging<Supplement> getPledgePageListByIdDesc(SysUserPageRequest sysUserPageRequest) {
        LambdaQueryWrapper<Supplement> wr = Wrappers.lambdaQuery(Supplement.class);
        Page<Supplement> page = new Page<>(sysUserPageRequest.getPageIndex(), sysUserPageRequest.getPageSize());
        if (sysUserPageRequest.getStartDate() != null) {
            wr.ge(Supplement::getCreateTime, sysUserPageRequest.getStartDate());
        }
        if (sysUserPageRequest.getEndDate() != null) {
            wr.le(Supplement::getCreateTime, sysUserPageRequest.getEndDate());
        }
        if (sysUserPageRequest.getId()!=null){
            wr.eq(Supplement::getUId,sysUserPageRequest.getId());
        }

        page = supplementService.page(page, wr);
        for (Supplement supplement:page.getRecords()) {
            //查询币种名称
            String assetName = sysUtilController.querySymbol(Integer.parseInt(supplement.getAssetId().toString()));
            if (StringUtils.isNotBlank(assetName)){
                supplement.setAssetName(assetName);
            }
        }
        Page<Supplement> objectPage = new Page<>();
        objectPage.setRecords(page.getRecords());
        objectPage.setTotal(page.getRecords().size());
        return new Paging<>(objectPage);
    }
    ...
    public Paging<AssetAccountLog> platformOperationList(AssetAccountLogPageParam assetAccountLogPageParam) {
        ...
        if (assetAccountLogPageParam.getAccount()!=null){
        ...
        //系统充值扣除过滤
        wr.eq(AssetAccountLog::getType,AccountLogType.TYPE_SYSTEM_RECHARGE).or(x->x.eq(AssetAccountLog::getType,AccountLogType.TYPE_SYSTEM_DELETE));
    ...
    @Override
    public BigDecimal getServiceFee(Long userId) throws Exception {
        User one = getById(userId);
        if (one.getServerCharge() == null || one.getServerCharge().compareTo(BigDecimal.ZERO) <= 0)
            return BigDecimal.ZERO;
        else
            return one.getServerCharge();
    }
}
