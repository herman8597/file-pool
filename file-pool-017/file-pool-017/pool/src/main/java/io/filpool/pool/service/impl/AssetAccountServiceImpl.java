...
//                List<CloudAccount> accounts = lambdaQuery().eq(CloudAccount::getUserId, user.getId()).list();
                List<Currency> list = currencyService.lambdaQuery().list();
                Date now = new Date();
                //判断是否生成了对应的账户
    ...
    public AssetAccountVo.AccountVo getAccount(Long currencyId) throws Exception {
        ...
        vo.setTotal(account.getAvailable().add(account.getFrozen()).add(account.getPledge()));
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Boolean exchangeAccount(AssetAccountExchangeVo assetAccountExchangeVo) {
        //查询用户当前的资产
