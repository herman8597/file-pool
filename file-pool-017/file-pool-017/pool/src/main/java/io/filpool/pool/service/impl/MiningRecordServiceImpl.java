...
//        record.setEndTime(request.getEndTime());
        ...
        if (request.getIsAgent() != null && request.getIsAgent()) {
            ...
            totalPower = powerOrderService.getBaseMapper().sumEffectAmount(one.getId());
            ...
            //普通用户
            //全网有效算力
            totalPower = powerOrderService.getBaseMapper().sumSystemPower();
            record.setPower(totalPower);
            record.setType(1);
            //可参与分币用户
            userIdList.addAll(powerOrderService.getBaseMapper().getUserIdList());
            //参与人数
        ...
        Integer day = releaseConfig != null ?Integer.parseInt(releaseConfig.getConfigValue()):180;
        if (config == null) {
            throw new FILPoolException("config.not-exits");
        }
        BigDecimal fee = new BigDecimal(config.getConfigValue());
        for (Long userId : userIdList) {
            ...
            //用户可用算力
            BigDecimal userPower = powerOrderService.getBaseMapper().sumEffectAmount(userId);
            //每个tb数量*手续费率*用户算力数量
            BigDecimal totalIncome = tbAmount.multiply(BigDecimal.ONE.subtract(fee)).multiply(userPower);
            //首次释放
            ...
            releaseRecord.setOnceAmount(frozenIncome.divide(new BigDecimal(day), 8, RoundingMode.DOWN).setScale(8, RoundingMode.DOWN));
            ...
            releaseRecord.setFrozenAmount(frozenIncome);
            ...
            //直接释放25%,产生资金记录
            account.setAvailable(account.getAvailable().add(firstIncome));
            accountService.updateAssetAccount(account);
            ...
            //产生一条首次释放记录
            incomeLogService.saveLog(account.getUserId(), 2, record.getId(),firstIncome);
            //产生一条挖矿收益
            incomeLogService.saveLog(account.getUserId(), 1, record.getId(),totalIncome);
            //产生一条冻结收益记录
            incomeLogService.saveLog(account.getUserId(), 3, record.getId(),frozenIncome);
        }
