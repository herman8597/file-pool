...
public interface InviteRecordMapper extends BaseMapper<InviteRecord> {
   ...
   /* @Select("SELECT * FROM(SELECT *,2 as relation FROM fil_invite_record WHERE invite_user_id in(SELECT user_id FROM fil_invite_record WHERE invite_user_id = #{inviteRecordRequest.id})\n" +
            "UNION\n" +
            "SELECT *,1 as relation FROM fil_invite_record WHERE invite_user_id =#{inviteRecordRequest.id}) AS a WHERE")*/
    List<InviteRecord> queryInviteRecord(@Param("InviteRecordRequest") InviteRecordRequest inviteRecordRequest);

    List<RewardDescVo> queryRewardDesc(@Param("InviteRecordPageParam") InviteRecordPageParam inviteRecordPageParam);
}
