...
import io.filpool.config.properties.SwaggerProperties;
import io.filpool.framework.common.exception.SpringBootPlusConfigException;
...
//                .apis(basePackage(basePackages))
                .apis(RequestHandlerSelectors.basePackage("io.filpool.pool.controller.app"))
    ...
    @Bean
    public Docket getSysApi() {
        String[] pages = {"io.filpool.pool.controller.admin", "io.filpool.system.controller"};
        return new Docket(DocumentationType.SWAGGER_2)
                .apiInfo(apiInfo())
                .select()
                .apis(basePackage(pages))
                .paths(PathSelectors.any())
                .build()
                .ignoredParameterTypes(ignoredParameterTypes)
                .globalOperationParameters(getParameters())
