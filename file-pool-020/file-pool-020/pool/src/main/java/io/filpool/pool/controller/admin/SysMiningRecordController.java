...
public class SysMiningRecordController extends BaseController {
    ...
    @PostMapping("/delMiningRecord")
    @OperationLog(name = "删除平台挖矿记录", type = OperationLogType.ADD)
    @ApiOperation(value = "删除平台挖矿记录", response = ApiResult.class)
    public ApiResult<Boolean> delMiningRecord(@RequestBody MiningRecord request) throws Exception {
        boolean b = miningRecordService.removeById(request.getId());
    ...
    public ApiResult<Paging<MiningRecord>> getAgentRecord(@RequestBody SysMiningPageRequest request) throws Exception {
        LambdaQueryWrapper<MiningRecord> wr = Wrappers.lambdaQuery(MiningRecord.class).orderByDesc(MiningRecord::getCreateTime);
     ...
     * 后台管理-平台币种数据统计
     */
    @PostMapping("/assetCountAll")
    @ApiOperation("平台币种数据统计")
    @Transactional
    public ApiResult<List<AssetCountAllVo>> assetCountAll() throws Exception {
        BigDecimal fil = powerOrderService.lambdaQuery().eq(PowerOrder::getPowerSymbol, "FIL").list().stream().map(PowerOrder::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal xch = powerOrderService.lambdaQuery().eq(PowerOrder::getPowerSymbol, "XCH").list().stream().map(PowerOrder::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal bzz = powerOrderService.lambdaQuery().eq(PowerOrder::getPowerSymbol, "BZZ").list().stream().map(PowerOrder::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add);

        List<AssetCountAllVo> list = new ArrayList<>();
        for(int i=0;i<3;i++){
            AssetCountAllVo assetCountAllVo = new AssetCountAllVo();
            if (i==0){
                assetCountAllVo.setQuantity(fil);
                assetCountAllVo.setSymbol("FIL");
            }else if (i==1){
                assetCountAllVo.setQuantity(xch);
                assetCountAllVo.setSymbol("XCH");
            }else if (i==2){
                assetCountAllVo.setQuantity(bzz);
                assetCountAllVo.setSymbol("BZZ");
            }
            list.add(assetCountAllVo);
        }
...
//    }
    ...
    public ApiResult<Paging<SysIncomeRecordVo>> getIncomeRecord(@RequestBody SysIncomeRecordRequest request) throws Exception {
        ...
        wr.orderByDesc(IncomeReleaseRecord::getCreateTime);
    ...
    @ApiOperation("释放记录")
    @Transactional
    public ApiResult<Paging<IncomeLog>> getReleaseLog(@RequestBody SysReleaseLogRequest request) throws Exception {
        if (request.getReleaseId() == null) {
            throw new FILPoolException("id.not-empty");
        }
        IncomeReleaseRecord one = incomeReleaseRecordService.lambdaQuery().eq(IncomeReleaseRecord::getId, request.getReleaseId()).one();
        Long userId = null;
        ...
        wr.orderByDesc(IncomeLog::getCreateTime);
        wr.eq(IncomeLog::getUserId, userId);
        wr.eq(IncomeLog::getPowerSymbol, one.getSymbol());
        if (StringUtils.equals(one.getSymbol(), "FIL")) {
            wr.in(IncomeLog::getType, 2, 4);
        } else {
            //奇亚只有1
            wr.in(IncomeLog::getType, 1);
        }
    ...
    public void getReleaseLogExcel(@RequestBody SysReleaseLogRequest request, HttpServletResponse response) throws Exception {
        ApiResult<Paging<IncomeLog>> releaseLog = getReleaseLog(request);

        //将数据循环存到导出实体类中
        List<IncomeRecordLogExcel> excelInviteRecordLogList = new ArrayList<>();

        for (IncomeLog incomeReleaseLog : releaseLog.getData().getRecords()) {
            ...
            if (incomeReleaseLog.getType() == 2 || incomeReleaseLog.getType() == 1) {
                incomeRecordLogExcel.setType("直接释放");
            } else {
                incomeRecordLogExcel.setType("线性释放");
            }
        }
        FileUtil.exportExcel(excelInviteRecordLogList, IncomeRecordLogExcel.class, "释放记录", response);
    }
