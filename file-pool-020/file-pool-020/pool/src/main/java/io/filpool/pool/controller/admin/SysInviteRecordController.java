...
public class SysInviteRecordController extends BaseController {
    ...
    @OperationLog(name = "邀请返佣奖励", type = OperationLogType.PAGE)
    @ApiOperation(value = "邀请返佣奖励", response = InviteRecord.class)
    ...
    @OperationLog(name = "邀请返佣奖励-导出", type = OperationLogType.PAGE)
    @ApiOperation(value = "邀请返佣奖励-导出", response = InviteRecord.class)
    public void getInviteRecordPageList(@Validated @RequestBody InviteRecordPageParam inviteRecordPageParam, HttpServletResponse response) throws Exception {
        ...
        for (InviteRecord inviteRecord:paging.getRecords()) {
            ...
            //处理导出时候显示的0E-8问题
            if (BigDecimalUtil.equal(inviteExcel.getFyTb(), BigDecimal.ZERO)){
                inviteExcel.setFyTb(BigDecimal.ZERO);
            }
            if (BigDecimalUtil.equal(inviteExcel.getFyUsdt(), BigDecimal.ZERO)){
                inviteExcel.setFyUsdt(BigDecimal.ZERO);
            }
        }
    ...
    public void inviteRecordDescPageListExcel(@RequestBody InviteRecordRequest inviteRecordRequest,HttpServletResponse response) throws Exception {
        ...
        for (InviteRecord inviteRecord:inviteRecordDescPageList.getRecords()) {
            ...
            if (inviteRecord.getRelation()==1){
                inviteRecordDescExcel.setRelation("直接");
            }
            if (inviteRecord.getRelation()==2){
                inviteRecordDescExcel.setRelation("间接");
            }

            inviteRecordDescExcel.setFyUsdt(inviteRecord.getFyUsdt().stripTrailingZeros().toPlainString());
            inviteRecordDescExcel.setFyTbFil(inviteRecord.getFyTbFil().stripTrailingZeros().toPlainString());
            inviteRecordDescExcel.setFyTbXch(inviteRecord.getFyTbXch().stripTrailingZeros().toPlainString());
    ...
    public void infoExcel(@Validated @RequestBody InviteRecordPageParam inviteRecordPageParam,HttpServletResponse response) throws Exception {
        Paging<RewardDescVo> inviteRecordDesc = inviteRecordService.getInviteRecordDesc(inviteRecordPageParam);
        for (RewardDescVo rewardDescVo:inviteRecordDesc.getRecords()) {
            //处理导出0E-8问题
            if (BigDecimalUtil.equal(rewardDescVo.getUsdtAmount(), BigDecimal.ZERO)){
                rewardDescVo.setUsdtAmount(BigDecimal.ZERO);
            }
            if (BigDecimalUtil.equal(rewardDescVo.getQuantity(),BigDecimal.ZERO)){
                rewardDescVo.setQuantity(BigDecimal.ZERO);
            }
            if (BigDecimalUtil.equal(rewardDescVo.getPowerAmount(),BigDecimal.ZERO)){
                rewardDescVo.setPowerAmount(BigDecimal.ZERO);
            }
        }
