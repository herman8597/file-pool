...
//    }
    ...
    public ApiResult<Paging<WithdrawRecord>> getWithdrawMoneyPageList(@Validated @RequestBody WithdrawRecordPageParam withdrawRecordPageParam){
        ...
        for (WithdrawRecord withdrawRecord:paging.getRecords()) {
            ...
            String symbol = withdrawRecord.getSymbol();
            AssetAccount one = assetAccountService.lambdaQuery().eq(AssetAccount::getUserId, userId).eq(AssetAccount::getSymbol, symbol).one();
            if (ObjectUtil.isNotEmpty(one)){
                withdrawRecord.setAvailable(one.getAvailable());
            }
        }
        return ApiResult.ok(paging);
    }
    ...
    public ApiResult<Boolean> auth(@RequestBody WithdrawRecord request) throws Exception {
        WithdrawRecord record = withdrawRecordService.getById(request.getId());
        //将后台修改的提币数量与手续费插入进去
            ...
            try {
                if (request.getStatus()==2) {
                    ...
                    if (active.equals("dev") /*|| active.equals("test")*/) {
                        ...
                        ApiResult<WithdrawVo> result = cloudApiRpc.transfer(address.getAddress(),address.getSecret(), record.getToAddress(), symbol,record.getAmount().subtract(record.getFee()),record.getId());
                        if (result.isSuccess()){
                    ...
                    //减去冻结
                    boolean isUpdate = assetAccountService.lambdaUpdate().eq(AssetAccount::getId, assetAccount.getId())
                            .setSql("frozen = frozen - " + record.getAmount().stripTrailingZeros().toPlainString())
                            .set(AssetAccount::getCreateTime,new Date())
                            .update();
                    if (!isUpdate) {
                        throw new FILPoolException("asset.modification");
                    }
                    //刷新账户
                    assetAccount = assetAccountService.lambdaQuery().eq(AssetAccount::getUserId, record.getUserId()).eq(AssetAccount::getSymbol, record.getSymbol()).one();
                    //更改提币记录
                    record.setStatus(5);
...
//                    accountLogService.saveLog(assetAccount, record.getAmount().negate(), AccountLogType.TYPE_WITHDRAW, "提币审核通过:" + hash,record.getId());
                    ...
                    //冻结返回
                    boolean isUpdate = assetAccountService.lambdaUpdate().eq(AssetAccount::getId, assetAccount.getId())
                            .setSql("available = available + " + record.getAmount().stripTrailingZeros().toPlainString())
                            .setSql("frozen = frozen - " + record.getAmount().stripTrailingZeros().toPlainString())
                            .set(AssetAccount::getCreateTime,new Date())
                            .update();
                    if (!isUpdate) {
