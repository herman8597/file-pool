...
public class GoodsServiceImpl extends BaseServiceImpl<GoodsMapper, Goods> implements GoodsService {
    ...
    public Paging<Goods> getGoodsPageList(GoodsPageParam goodsPageParam) throws Exception {
        ...
        if (goodsPageParam.getStartDate() != null) {
            wrapper.ge(Goods::getCreateTime, goodsPageParam.getStartDate());
        }
        if (goodsPageParam.getEndDate() != null) {
            wrapper.le(Goods::getCreateTime, goodsPageParam.getEndDate());
        }
    ...
    public boolean saveOrUpdateGood(Goods goods) throws Exception {
        Date date = new Date();
        if (goods.getServingFee() == null) {
            goods.setServingFee(BigDecimal.ZERO);
        }
        if (goods.getId() != null){
            //修改商品
