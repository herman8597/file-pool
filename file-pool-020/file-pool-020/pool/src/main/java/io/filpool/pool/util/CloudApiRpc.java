...
//            return "";
    ...
    public List<RechargeRecord> getNewRecharge(String address, String secret, Date time) throws Exception {
        ...
        Request request = getRequest("getNewRecharge", gson.toJson(params));
        Response response = getOkHttpClient().newCall(request).execute();
        String json = response.body().string();
//        log.info("getNewRecharge json:{}", json);
...
//        HashMap<String, Object> params = new HashMap<>();
//        params.put("pid", recordId);
        Request request = getRequest("getWithdrawLogByPid?pid=" + recordId, "{}");
        Response response = getOkHttpClient().newCall(request).execute();
        String json = response.body().string();
//        log.info("json:{}", json);
        ApiResult<RechargeRecord> result = gson.fromJson(json, new TypeToken<ApiResult<RechargeRecord>>() {
        }.getType());
        if (result.isSuccess())
            return result.getData().getTransHash();
        else
