...
@Slf4j
@RestController
@RequestMapping(Constants.API_PREFIX +"/appVersion")
@Module("pool")
...
public class AppVersionController extends BaseController {
     ...
     * app版本更新(获取最新版本)
     */
    @PostMapping("/getNewestVersion")
    @OperationLog(name = "app版本更新", type = OperationLogType.PAGE)
    @ApiOperation(value = "app版本更新", response = AppVersion.class)
    public ApiResult<AppVersion> getNewestVersion(@Validated @RequestBody AppVersionPageParam appVersionPageParam) throws Exception {
        AppVersion appVersion = appVersionService.getNewestVersion(appVersionPageParam);
        return ApiResult.ok(appVersion);
    }
